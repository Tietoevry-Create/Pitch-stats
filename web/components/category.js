...
const Category = ({ category, ...rest }) => (
  <CustomLink to={category}>
    <a className="pb-6 pt-16 px-3 rounded-lg border-2 border-yellow-50 hover:border-tepurple hover:bg-tepurple/20 hover:scale-110 group">
      <div className="items-center md:items-start grid grid-cols-2 gap-3">
        <svg
          viewBox="0 0 24 24"
        ...
        <div className="grid grid-row-2 gap-7">
          ...
          <p className="hidden md:block text-lg md:text-xl ">{category.lede}</p>
        </div>
      </div>
      <p className="md:hidden text-lg md:text-xl rounded-lg mb-auto border-2 border-tepurple py-5 px-5 md:py-10 md:px-16">
        {category.lede}
      </p>
    </a>
  </CustomLink>
