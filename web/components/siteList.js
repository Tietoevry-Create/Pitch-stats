		...
		<div ref={refs} className={'container mx-auto px-4 md:px-24 py-24 text-xl'}>
			<section>
				...
				<ul ref={siteListRef}>
					{siteList &&
					siteList.length > 0 &&
					siteList.slice(0,size).map((site, index) => <SingleSite index={index} site={site} key={site._id} />)}
				</ul>
				<div className={`${!showLoadMore ? 'hidden' : ''}` + " container flex flex-wrap items-center justify-center mx-auto mt-5 w-full"}>
					<button onClick={() => increaseSiteList()} className={"px-3 py-2 text-black font-bold rounded-lg border-2 border-yellow-50 hover:border-tepurple hover:bg-tepurple/20"}>
						Load More
					</button>
				</div>
			</section>
